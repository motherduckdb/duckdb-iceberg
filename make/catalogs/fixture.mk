...
fixture-data: fixture
	...
	python3 -m pytest scripts/data_generators/test_generate_data.py --catalog fixture --spark-runtime 4.0 $(if $(TEST),-k $(TEST)) -vv
...
fixture-data-local: fixture
	...
	python3 -m pip install -r scripts/requirements.txt && \
	if [ -f "$(FIXTURE_ENV_FILE)" ]; then echo "Loading env from $(FIXTURE_ENV_FILE)"; set -a; . ./$(FIXTURE_ENV_FILE); set +a; fi && \
	python3 -m pytest scripts/data_generators/test_generate_data.py --catalog local --spark-runtime 4.0 $(if $(TEST),-k $(TEST)) -vv
