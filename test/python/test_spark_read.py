...
class TestSparkRead:
    ...
    def test_spark_time_travel_rowid(self, spark_con):
        df = spark_con.sql(
            """
            select _row_id, _last_updated_sequence_number, * from default.row_lineage_test_upgraded_insert order by id
            """
        )
        res = df.collect()
        assert res == [
            Row(_row_id=3, _last_updated_sequence_number=5, id=1, data='replaced'),
            Row(_row_id=0, _last_updated_sequence_number=8, id=2, data='replaced_again'),
            Row(_row_id=7, _last_updated_sequence_number=2, id=4, data='d_u1'),
            Row(_row_id=1, _last_updated_sequence_number=8, id=6, data='replaced_again'),
            Row(_row_id=2, _last_updated_sequence_number=7, id=7, data='g_new'),
        ]

        df = spark_con.sql(
            """
            SELECT snapshot_id, committed_at
            FROM default.row_lineage_test_upgraded_insert.snapshots
            ORDER BY committed_at
        """
        )
        snapshots = df.collect()
        assert snapshots == [
            Row(snapshot_id=1411754971390006631, committed_at=datetime.datetime(2026, 2, 27, 12, 57, 18, 245000)),
            Row(snapshot_id=7453172534201894462, committed_at=datetime.datetime(2026, 2, 27, 12, 57, 18, 999000)),
            Row(snapshot_id=477252100687316191, committed_at=datetime.datetime(2026, 2, 27, 12, 57, 19, 486000)),
            Row(snapshot_id=4295970171215436999, committed_at=datetime.datetime(2026, 2, 27, 12, 57, 19, 802000)),
            Row(snapshot_id=7419864520212302743, committed_at=datetime.datetime(2026, 2, 27, 12, 57, 20, 223000)),
            Row(snapshot_id=6946056827717817146, committed_at=datetime.datetime(2026, 2, 27, 12, 57, 20, 555000)),
            Row(snapshot_id=4020338888586457821, committed_at=datetime.datetime(2026, 2, 27, 12, 57, 20, 845000)),
            Row(snapshot_id=6055250224963931037, committed_at=datetime.datetime(2026, 2, 27, 12, 57, 21, 458000)),
        ]
        ...
        df = spark_con.sql(
            ...
            VERSION AS OF {snapshots[5]["snapshot_id"]}
            ORDER BY id
        """
        )
        ...
        res = df.collect()
        assert res == [
            Row(_row_id=None, _last_updated_sequence_number=None, id=1, data='replaced'),
            Row(_row_id=None, _last_updated_sequence_number=None, id=2, data='b_u1'),
            Row(_row_id=None, _last_updated_sequence_number=None, id=4, data='d_u1'),
            Row(_row_id=None, _last_updated_sequence_number=None, id=6, data='replaced'),
        ]
