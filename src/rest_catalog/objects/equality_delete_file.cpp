...
error = content_file.TryFromJSON(obj);if (!error.empty()) {
	...
	if (!content_val) {
		...
		if (yyjson_is_str(content_val)) {
			...
			return StringUtil::Format("EqualityDeleteFile property 'content' is not of type 'string', found '%s' instead", yyjson_get_type_desc(content_val));
		}
	}
	...
	if (equality_ids_val && !yyjson_is_null(equality_ids_val)) {
		...
		if (yyjson_is_arr(equality_ids_val)) {
			...
			if (yyjson_is_int(val)) {
				...
				return StringUtil::Format("EqualityDeleteFile property 'tmp' is not of type 'integer', found '%s' instead", yyjson_get_type_desc(val));
			}
				equality_ids.emplace_back(std::move(tmp));
			}
		} else {
			return StringUtil::Format("EqualityDeleteFile property 'equality_ids' is not of type 'array', found '%s' instead", yyjson_get_type_desc(equality_ids_val));
		}
	}
	return string();
}
