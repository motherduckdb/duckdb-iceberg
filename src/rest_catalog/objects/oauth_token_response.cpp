...
string OAuthTokenResponse::TryFromJSON(yyjson_val *obj) {
	...
	if (!access_token_val) {
		...
		if (yyjson_is_str(access_token_val)) {
			...
			return StringUtil::Format("OAuthTokenResponse property 'access_token' is not of type 'string', found '%s' instead", yyjson_get_type_desc(access_token_val));
		}
	}
	...
	if (!token_type_val) {
		...
		if (yyjson_is_str(token_type_val)) {
			...
			return StringUtil::Format("OAuthTokenResponse property 'token_type' is not of type 'string', found '%s' instead", yyjson_get_type_desc(token_type_val));
		}
	}
	...
	if (expires_in_val && !yyjson_is_null(expires_in_val)) {
		...
		if (yyjson_is_int(expires_in_val)) {
			...
			return StringUtil::Format("OAuthTokenResponse property 'expires_in' is not of type 'integer', found '%s' instead", yyjson_get_type_desc(expires_in_val));
		}
	}
	...
	if (refresh_token_val && !yyjson_is_null(refresh_token_val)) {
		...
		if (yyjson_is_str(refresh_token_val)) {
			...
			return StringUtil::Format("OAuthTokenResponse property 'refresh_token' is not of type 'string', found '%s' instead", yyjson_get_type_desc(refresh_token_val));
		}
	}
	...
	if (scope_val && !yyjson_is_null(scope_val)) {
		...
		if (yyjson_is_str(scope_val)) {
			...
			return StringUtil::Format("OAuthTokenResponse property 'scope' is not of type 'string', found '%s' instead", yyjson_get_type_desc(scope_val));
		}
	}
	return string();
}
