...
string SQLViewRepresentation::TryFromJSON(yyjson_val *obj) {
	...
	if (!type_val) {
		...
		if (yyjson_is_str(type_val)) {
			...
			return StringUtil::Format("SQLViewRepresentation property 'type' is not of type 'string', found '%s' instead", yyjson_get_type_desc(type_val));
		}
	}
	...
	if (!sql_val) {
		...
		if (yyjson_is_str(sql_val)) {
			...
			return StringUtil::Format("SQLViewRepresentation property 'sql' is not of type 'string', found '%s' instead", yyjson_get_type_desc(sql_val));
		}
	}
	...
	if (!dialect_val) {
		...
		if (yyjson_is_str(dialect_val)) {
			...
			return StringUtil::Format("SQLViewRepresentation property 'dialect' is not of type 'string', found '%s' instead", yyjson_get_type_desc(dialect_val));
		}
	}
	return string();
}
