...
if (yyjson_is_arr(obj)) {
	...
	if (yyjson_is_str(val)) {
		...
		return StringUtil::Format("Namespace property 'tmp' is not of type 'string', found '%s' instead", yyjson_get_type_desc(val));
	}
		value.emplace_back(std::move(tmp));
	}
} else {
	return StringUtil::Format("Namespace property 'value' is not of type 'array', found '%s' instead", yyjson_get_type_desc(obj));
}
	return string();
}
