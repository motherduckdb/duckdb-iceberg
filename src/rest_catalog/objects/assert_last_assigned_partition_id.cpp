...
string AssertLastAssignedPartitionId::TryFromJSON(yyjson_val *obj) {
	...
	if (!last_assigned_partition_id_val) {
		...
		if (yyjson_is_int(last_assigned_partition_id_val)) {
			...
			return StringUtil::Format("AssertLastAssignedPartitionId property 'last_assigned_partition_id' is not of type 'integer', found '%s' instead", yyjson_get_type_desc(last_assigned_partition_id_val));
		}
	}
	return string();
}
