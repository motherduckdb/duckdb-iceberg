...
error = base_update.TryFromJSON(obj);if (!error.empty()) {
	...
	if (!ref_name_val) {
		...
		if (yyjson_is_str(ref_name_val)) {
			...
			return StringUtil::Format("RemoveSnapshotRefUpdate property 'ref_name' is not of type 'string', found '%s' instead", yyjson_get_type_desc(ref_name_val));
		}
	}
	...
	if (action_val && !yyjson_is_null(action_val)) {
		...
		if (yyjson_is_str(action_val)) {
			...
			return StringUtil::Format("RemoveSnapshotRefUpdate property 'action' is not of type 'string', found '%s' instead", yyjson_get_type_desc(action_val));
		}
	}
	return string();
}
