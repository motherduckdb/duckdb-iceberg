...
vector<IcebergManifest> IcebergTable::ReadManifestListFile(const string &path, FileSystem &fs,
...
string GenerateMetaDataUrl(FileSystem &fs, const string &meta_path, string &table_version,
                           string &metadata_compression_codec, string &version_format = DEFAULT_TABLE_VERSION_FORMAT) {
	...
	throw IOException(
	    ...
	    table_version, metadata_compression_codec, version_format);
}

string IcebergSnapshot::GetMetaDataPath(ClientContext &context, const string &path, FileSystem &fs,
                                        string metadata_compression_codec, string table_version = DEFAULT_TABLE_VERSION,
                                        string version_format = DEFAULT_TABLE_VERSION_FORMAT) {
	string version_hint;
...
string IcebergSnapshot::GuessTableVersion(const string &meta_path, FileSystem &fs, string &table_version,
                                          string &metadata_compression_codec,
                                          string &version_format = DEFAULT_TABLE_VERSION_FORMAT) {
	OpenFileInfo selected_metadata;
	...
	for (auto try_format : StringUtil::Split(version_format, ',')) {
		...
		if (found_versions.size() > 0) {
			...
			if (!selected_metadata.path.empty()) { // Found one
				return selected_metadata.path;
			}
		}
	}
...
OpenFileInfo IcebergSnapshot::PickTableVersion(vector<OpenFileInfo> &found_metadata, string &version_pattern,
                                               string &glob) {
